...
from advision.openenv.tasks import TASKS
from advision.openenv.models import Observation, Action
from advision.env.reward import RewardComponents, REWARD_WEIGHTS
...
def _heuristic(obs):
    ...
    best=int(np.argmax(scores)); n=len(obs.surfaces)
    si=float(best)/max(1,n-1) if n>1 else 0.5
    ...
    return Action(surface_idx=float(np.clip(si,0,1)),
...
def run_task(task_id, n_frames=30, client=None, seed=42, video_path=None, ad_paths=None):
    env=AdPlacementEnv(max_frames=n_frames,seed=seed,video_path=video_path,ad_paths=ad_paths or [])
    task=TASKS[task_id](); task.reset()
    ...
    for step in range(n_frames):
        ...
        obs=Observation.from_vector(obs_vec.tolist(),step+1,info.get("n_persons",0))
        task.update(reward,info); rewards.append(float(reward))
        if term or trunc: break
    result=task.grade(); env.close()
...
def main(seed=42, video_path=None, ad_paths=None):
    api_key=os.environ.get("ANTHROPIC_API_KEY","").strip(); client=None
    if HAS_ANTHROPIC and api_key:
        client=anthropic.Anthropic(api_key=api_key); agent_name="Claude Haiku Agent"
    else:
        agent_name="Heuristic Agent (7-dim)"
    task_configs=[("task1",15,"Easy"),("task2",25,"Medium"),("task3",35,"Hard")]
    print("\n"+"="*60)
    print(f"ADVISION AI BASELINE | Agent: {agent_name} | Seed: {seed}")
    print("="*60)
    results={}
    for tid,nf,diff in task_configs:
        r=run_task(tid,nf,client,seed,video_path,ad_paths); results[tid]=r
        status="✅ PASS" if r["passed"] else "❌ FAIL"
        print(f"  [{diff}] {tid}: score={r['score']:.4f} {status} | mean_r={r['mean_reward']:.4f}")
    scores=[results[t]["score"] for t,_,_ in task_configs]
    print("="*60); print(f"  Average: {np.mean(scores):.4f}"); print("="*60)
    summary={"agent":agent_name,"seed":seed,"tasks":results,
             "avg_score":round(float(np.mean(scores)),4),
             "timestamp":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime())}
    os.makedirs("logs",exist_ok=True)
    with open("logs/baseline_results.json","w") as f: json.dump(summary,f,indent=2)
    print("✅ Saved → logs/baseline_results.json")
    return summary

if __name__=="__main__":
    import argparse; p=argparse.ArgumentParser()
    p.add_argument("--seed",type=int,default=42)
    args=p.parse_args(); main(seed=args.seed)

print('✅ baseline_agent.py v5 — 7-dim heuristic + Claude prompt updated')
