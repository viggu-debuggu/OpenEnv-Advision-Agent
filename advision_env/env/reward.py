...
class RewardComponents:
    ...
    def to_dict(self):
        return {
            k: round(float(getattr(self, k)), 4)
...
class RewardFunction:
    ...
    def compute(self, frame_before, frame_after, ad_mask, surface_mask,
        ...
        r_val = self._clamp(self._realism(frame_before, frame_after, ad_mask))
        a_val = self._clamp(self._alignment(ad_mask, surface_mask))
        l_val = self._clamp(self._lighting(frame_before, frame_after, ad_mask))
        o_val = self._clamp(self._occlusion(surface_depth, persons, ad_mask))
        v_val = self._clamp(self._visibility(ad_mask, frame_before.shape[:2]))
        t_val = self._clamp(self._temporal(ad_mask, corners))
        W = REWARD_WEIGHTS
        total = (W['realism']*r_val + W['alignment']*a_val + W['lighting']*l_val +
                 W['occlusion']*o_val + W['visibility']*v_val + W['temporal']*t_val)
        self.prev_mask = ad_mask.copy()
        self.prev_corners = corners.copy() if corners is not None else None
        return RewardComponents(r_val, a_val, l_val, o_val, v_val, t_val, self._clamp(total))
    ...
    def _occlusion(self, surf_depth, persons, ad_mask):
        depth_score = min(1., surf_depth / 0.65)
        penalty = 0.
