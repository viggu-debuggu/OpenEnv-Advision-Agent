...
from advision_env.models.vision_models import (  # noqa: E402
...
class AdPlacementEnv(gym.Env):
    ...
    Observation: 28-dim float32 Box
      ...
      [4]    edge density
      [5]    sharpness
      [6:26] 5 surfaces x 4 dims (cx, cy, area, depth)
    ...
    def _load_ads(self):
        ...
        if not self._ad_images:
            self._ad_images = [self._default_ad()]

    def _default_ad(self):
        ad = np.zeros((100, 200, 3), np.uint8)
        ad[:, :] = (0, 120, 255)
    ...
    def reset(self, seed=None, options=None):
        ...
        self._prev_good_surfaces = []
        self._prev_fkey = b''
        self._locked_si = None
        self._locked_ai = None
        self.engine.reset()
        self.reward_fn = RewardFunction()
    ...
    def step(self, action):
        ...
        surface = self._surfaces[si]
        ad_img = self._ad_images[ai]
    ...
    def _analyze(self):
        ...
        if self._frame_idx % 5 == 0 or not self._prev_good_surfaces:
            ...
            if detected:
                self._surfaces = detected
                self._using_mock = False
                self._prev_good_surfaces = detected
            elif self._prev_good_surfaces:
                self._surfaces = self._prev_good_surfaces
                self._using_mock = False
            else:
            ...
            seg_mask = self._scene.segmentation_mask if self._scene else np.zeros((self._frame.shape[0], self._frame.shape[1]), np.uint8)
    ...
    def _obs(self):
        ...
        obs[3] = float(self._frame.mean())/255
        obs[4] = edge_density
        obs[5] = sharpness
